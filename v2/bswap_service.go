...
type GetPoolConfigureLiquidity struct {
	ConstantA         string     `json:"constantA"`
	MinRedeemShare    string `json:"minRedeemShare"`
	SlippageTolerance string `json:"slippageTolerance"`
}

type GetPoolConfigureAssetConfigure struct {
...
type GetPoolConfigure struct {
	PoolId     int    `json:"poolId"`
	PoolNmae   string `json:"poolNmae"`
	UpdateTime int64  `json:"updateTime"`

	Liquidity GetPoolConfigureLiquidity `json:"liquidity"`
	AssetConfigure map[string]GetPoolConfigureAssetConfigure `json:"assetConfigure"`
}

func (bs *GetPoolConfigureService) PoolId(poolId int64) *GetPoolConfigureService{
	bs.poolId = &poolId
	return bs
}

func (bs *GetPoolConfigureService) RecvWindow(recvWindow int64) *GetPoolConfigureService{
	bs.recvWindow = &recvWindow
	return bs
}

func (bs *GetPoolConfigureService) Timestamp(timestamp int64) *GetPoolConfigureService{
	bs.timestamp = &timestamp
	return bs
}

func (bs *GetPoolConfigureService) Do(ctx context.Context, opts ...RequestOption) ([]*GetPoolConfigure, error) {
	r := &request{
		method:   "GET",
	...
	if bs.recvWindow != nil {
		r.setParam("recvWindow", *bs.recvWindow)
	}

	if bs.timestamp != nil {
		r.setParam("timestamp", *bs.timestamp)
	}
